...
struct DataModel::Impl {
  Impl(DataModel &that, const char *path) : this_{that}, dataProvider_{path} {
  ...
  void compress(int index) {
    ...
    log_info << "Prepearing to compress \""
             << dirData_[index].first.baseName().toStdString() << "\"";
  ...
  void decompress(int index) {
    ...
    log_info << "Prepearing to decompress \""
             << dirData_[index].first.baseName().toStdString() << "\"";
...
DataModel::DataModel(const char *path, QObject *parent)
    : AbstractDataModel{parent}, impl_{std::make_unique<Impl>(*this, path)} {
